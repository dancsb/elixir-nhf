defmodule Nhf1TesztExs do

  def eval(f, tss) do
    r = (to_int(f) |> Nhf1.satrak() |> Enum.sort) |> (IO.inspect limit: :infinity)
    (r === Enum.sort(tss)) |> IO.inspect
  end

  def to_int(f) do
    mx = File.read!(f) |> String.trim() |> String.split("\n")
    row2 = hd(mx) |> String.trim() |> String.split(~r{\s+})
    {row1, mss} = (for rs <- tl(mx), zs = (rs |> String.trim() |> String.split(~r{\s+})), do:
                                                                          {hd(zs), tl(zs)}) |> Enum.unzip()
    { Enum.map(row1, &String.to_integer()/1),
      Enum.map(row2, &String.to_integer()/1),
      (for i <- 1..length(mss), j <- 1..length(hd(mss)), Enum.at(Enum.at(mss, i-1), j-1) == "*", do: {i,j})
    }
  end
end

  IO.puts "--- 01dpty"
  Nhf1TesztExs.eval("t01dpty.txt", [])

  IO.puts "--- 01dcol"
  Nhf1TesztExs.eval("t01dcol.txt", [[:n, :n], [:n, :s], [:s, :s]])

  IO.puts "--- 01drow"
  Nhf1TesztExs.eval("t01drow.txt", [[:e, :e], [:w, :e], [:w, :w]])

  IO.puts "--- 01d"
  Nhf1TesztExs.eval("test01d.txt", [[:w, :n, :e]])

  IO.puts "--- 03d"
  Nhf1TesztExs.eval("test03d.txt", [[:n, :e, :s, :w], [:n, :s, :n, :e], [:s, :n, :n, :e]])

  IO.puts "--- 06d"
  Nhf1TesztExs.eval("test06d.txt", [[:e, :e, :e, :s, :n, :w, :w, :s, :n, :w, :e, :s, :e, :e, :n]])

  IO.puts "--- 07d"
  Nhf1TesztExs.eval("test07d.txt",
    [
  ...
  Nhf1TesztExs.eval("test18d.txt", t18s)
